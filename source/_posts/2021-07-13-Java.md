...
### 线程生命周期和状态
* 初始：new 了一个线程后还没调用 start() 时的状态。
* 运行态：线程调用 start() 后进入就绪队列（就绪态）；或者在就绪队列中被分配到 CPU 时间片（运行中）。这两种状态都笼统地称为运行态。
* 阻塞：线程阻塞于锁（synchronized 修饰的方法或代码块）。
* 等待：调用 wait() 等方法进入该状态，需要等待其他线程做出一些动作如 notify() 或者中断，否则无限期等待。
* 超时等待：调用 sleep() 进入该状态，在计时器结束后自行继续。
* 终止：线程执行 run() 方法完毕。
